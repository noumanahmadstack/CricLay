...
import { initialState, subtractSyncBallsCount } from '../../../redux/scoring/normal/score/reducer';
...
const Scoring: FC<any> = ({ route, navigation }) => {
    ...
    const { lineupData, isLoading, matchDetail } = useSelector((state: RootState) => state.scoreReducer);
    ...
    const totalExtras = useMemo<number>(() => calculateExtras({ balls }), [balls]);
    const currentOver = useMemo<number>(() => calculateOvers({ balls, ballsPerOver }), [balls, ballsPerOver]);
    const currentRuns = useMemo<number>(() => calculateRuns({ balls }), [balls]);
    const currentRunRate = useMemo<string>(() => calculateCCR({ balls, ballsPerOver, currentRuns }), [balls, ballsPerOver, currentRuns]);
