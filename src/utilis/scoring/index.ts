...
import {onAddBalls} from '../../redux/scoring/amateur/score/reducer';
...
export const calculateExtras = ({balls}: {balls: BallObjProps[]}): number => {
  if (balls.length > 0) {
    return balls.reduce((sum, ball) => {
      ...
      } else if (ball.extrasType == 'no_ball' && !isByeLegBye) {
        return sum + 1;
      }
      return sum;
    }, 0);
...
export const calculateMedianOver = (bowlerId?: string) => {
  const {balls, ballsPerOver} =
    store.getState().scoreReducer.matchDetail.currentInning;
  let medanOver = 0;
